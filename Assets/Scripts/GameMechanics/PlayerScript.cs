...
public class PlayerScript : uLink.MonoBehaviour
{
	...
	public bool lockMouse = true; // it's set to true in start() anyway
    ...
    public Bounds Bounds { get { return ObjectToUseForBounds.GetComponent<Renderer>().bounds; } }
        ...
        set
        {
            ...
            if (needsUpdate)
            {
                ...
                if (uLink.Network.isServer)
                    networkView.RPC("RemoteReceiveHasFlagVisible", uLink.RPCMode.Others, _HasFlagVisible);
...
// ReSharper disable once UnusedMember.Local
    ...
    public void OnGUI()
    {
        ...
        {
			lockMouse = !lockMouse;//false; // ultra pro mouse fix
        }
    }
    ...
    public void GetTargetedBy(PlayerScript enemy)
    {
        ...
        {
            networkView.RPC("RemoteReceiveTargetedBy", uLink.RPCMode.Others, enemy.networkView.viewID);
        }
    }
    ...
    public void GetUntargetedBy(PlayerScript enemy)
    {
        ...
        if (ShouldSendMessages)
        {
            networkView.RPC("RemoteReceiveUntargetedBy", uLink.RPCMode.Others, enemy.networkView.viewID);
        }
    }
